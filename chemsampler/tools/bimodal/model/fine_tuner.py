...
class FineTuner():
    ...
    def __init__(self, experiment_name='ForwardRNN'):
        ...
        self.store_dir = '../evaluation'

    def fine_tuning(self, model_name, restart=False):
        ...
        # Create directories
        stor_dir= self.store_dir
        data_ = model_name + '.csv'
        data_file = os.path.join('../data' , data_)
        _data =  self._encoder.encode_from_file(data_file)
        ...
        # Compute labels
        label = np.argmax(_data, axis=-1).astype(int)
        ...
        if self._model_type == 'NADE' and self._generation == 'random':
            ...
            label = np.argmax(_data[:, 0], axis=-1).astype(int)
            aug = _data.shape[1] - 1
        ...
        for i in range(self._epochs):
            ...
            statistic = self._model.train(_data.reshape(-1, self._molecular_size, self._encoding_size),
            ...
            # Store model
            self._model.save(
                stor_dir + '/' + self._experiment_name + '/models/'+ model_name + 'model_fold_' + str(fold) + '_epochs_' + str(i) )
            ...
            pd.DataFrame(new_molecules).to_csv(
                stor_dir + '/' + self._experiment_name + '/molecules/' + model_name +'molecule_fold_' + str(fold) + '_epochs_' + str(
                    i) + '.csv', header=None)
